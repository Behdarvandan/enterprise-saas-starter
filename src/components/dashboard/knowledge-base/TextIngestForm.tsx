...
export default function TextIngestForm({ onSubmit }: TextIngestFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      <section aria-label={t("preview.title")} className="rounded-lg border border-border bg-muted/40 p-3">
        <div className="flex items-baseline justify-between gap-3">
          <h3 className="text-xs font-semibold text-foreground">{t("preview.title")}</h3>
          {chunks.length > 0 ? (
        ...
        {chunks.length === 0 ? (
          <p className="mt-2 text-xs text-muted-foreground">{t("preview.empty")}</p>
        ) : (
          <ol className="mt-3 grid gap-2">
            {chunks.slice(0, PREVIEW_LIMIT).map((chunk) => (
              <li key={chunk.index} className="rounded-md border border-border bg-secondary/60 p-2.5">
                ...
                <p className="mt-1 line-clamp-3 text-xs whitespace-pre-wrap text-foreground">{chunk.content}</p>
              </li>
            ))}
